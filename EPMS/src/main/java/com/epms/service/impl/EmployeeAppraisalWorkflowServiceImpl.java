...
public class EmployeeAppraisalWorkflowServiceImpl implements EmployeeAppraisalWorkflowService {
    ...
    public EmployeeAppraisalFormResponse createPmDraft(Integer cycleId, Integer employeeId, Integer pmUserId) {
        ...
        addHistory(saved, null, EmployeeAppraisalStatus.PM_DRAFT, pmUser, "CREATE_PM_DRAFT", "PM draft created.");
        return mapForm(saved);
    }

    @Override
    public EmployeeAppraisalFormResponse submitPmReview(Integer employeeAppraisalFormId, PmAppraisalSubmitRequest request, Integer pmUserId) {
        if (request == null || request.getRatings() == null || request.getRatings().isEmpty()) {
            throw new BadRequestException("PM ratings are required.");
        }
        EmployeeAppraisalForm form = getFormEntity(employeeAppraisalFormId);
        if (form.getStatus() != EmployeeAppraisalStatus.PM_DRAFT && form.getStatus() != EmployeeAppraisalStatus.RETURNED) {
            throw new BadRequestException("Only PM draft or returned forms can be submitted by PM.");
        }
        User pmUser = getUser(pmUserId);
        form.setProjectManager(pmUser);
        ...
        upsertReview(form, AppraisalReviewStage.PM, pmUser, request.getRecommendation(), request.getComment(), AppraisalDecision.APPROVED);
        ...
        EmployeeAppraisalForm saved = formRepository.save(form);
        addHistory(saved, previousStatus, EmployeeAppraisalStatus.DEPT_HEAD_PENDING, pmUser, "SUBMIT_TO_DEPT_HEAD", "PM submitted appraisal to Dept Head.");
        return mapForm(saved);
    }

    @Override
    public EmployeeAppraisalFormResponse submitDeptHeadReview(Integer employeeAppraisalFormId, AppraisalReviewSubmitRequest request, Integer deptHeadUserId) {
        EmployeeAppraisalForm form = getFormEntity(employeeAppraisalFormId);
        ...
        upsertReview(form, AppraisalReviewStage.DEPT_HEAD, deptHead, safeRecommendation(request), safeComment(request), AppraisalDecision.APPROVED);

        EmployeeAppraisalStatus previousStatus = form.getStatus();
        form.setStatus(EmployeeAppraisalStatus.HR_PENDING);
        form.setDeptHeadSubmittedAt(new Date());
        EmployeeAppraisalForm saved = formRepository.save(form);
        addHistory(saved, previousStatus, EmployeeAppraisalStatus.HR_PENDING, deptHead, "SUBMIT_TO_HR", "Dept Head reviewed and submitted appraisal to HR.");
        return mapForm(saved);
    }

    @Override
    public EmployeeAppraisalFormResponse approveByHr(Integer employeeAppraisalFormId, AppraisalReviewSubmitRequest request, Integer hrUserId) {
        EmployeeAppraisalForm form = getFormEntity(employeeAppraisalFormId);
        ...
        upsertReview(form, AppraisalReviewStage.HR, hrUser, safeRecommendation(request), safeComment(request), AppraisalDecision.APPROVED);
        ...
        form.setLocked(true);
        EmployeeAppraisalForm saved = formRepository.save(form);
        addHistory(saved, previousStatus, EmployeeAppraisalStatus.COMPLETED, hrUser, "HR_APPROVE", "HR approved appraisal. Employee can view the form.");
        return mapForm(saved);
    }

    @Override
    public EmployeeAppraisalFormResponse returnToPm(Integer employeeAppraisalFormId, String note, Integer actionByUserId) {
        EmployeeAppraisalForm form = getFormEntity(employeeAppraisalFormId);
        if (form.getStatus() != EmployeeAppraisalStatus.DEPT_HEAD_PENDING && form.getStatus() != EmployeeAppraisalStatus.HR_PENDING) {
            throw new BadRequestException("Only Dept Head pending or HR pending forms can be returned to PM.");
        }
        ...
        form.setLocked(false);
        EmployeeAppraisalForm saved = formRepository.save(form);
        addHistory(saved, previousStatus, EmployeeAppraisalStatus.RETURNED, user, "RETURN_TO_PM", note);
        return mapForm(saved);
    }
    ...
    public List<EmployeeAppraisalFormResponse> getPmHistory(Integer pmUserId) {
        return formRepository.findByProjectManagerIdOrderByUpdatedAtDesc(pmUserId).stream()
                .map(this::mapForm)
                .toList();
    }
    ...
    public List<EmployeeAppraisalFormResponse> getDeptHeadQueue(Integer departmentId) {
        return formRepository.findByDepartmentIdAndStatus(departmentId, EmployeeAppraisalStatus.DEPT_HEAD_PENDING).stream()
                .map(this::mapForm)
                .toList();
    }
    ...
    public List<EmployeeAppraisalFormResponse> getDeptHeadHistory(Integer deptHeadUserId) {
        return formRepository.findByDepartmentHeadIdOrderByUpdatedAtDesc(deptHeadUserId).stream()
                .map(this::mapForm)
                .toList();
    }
    ...
    public List<EmployeeAppraisalFormResponse> getHrReviewQueue() {
        return formRepository.findByStatus(EmployeeAppraisalStatus.HR_PENDING).stream()
                .map(this::mapForm)
                .toList();
    }
    ...
    public List<EmployeeAppraisalFormResponse> getEmployeeVisibleForms(Integer employeeId) {
        return formRepository.findByEmployeeIdAndVisibleToEmployeeTrueOrderByHrApprovedAtDesc(employeeId).stream()
                .map(this::mapForm)
                .toList();
    }
    ...
    private AppraisalCycle getActiveCycle(Integer cycleId) {
        ...
        if (cycle.getStatus() != AppraisalCycleStatus.ACTIVE) {
            throw new BadRequestException("Only active appraisal cycles can be used for PM review.");
        }
        if (Boolean.TRUE.equals(cycle.getLocked())) {
            throw new BadRequestException("Locked appraisal cycles cannot be used for PM review.");
        }
        return cycle;
    }
    ...
    private Department resolveEmployeeDepartment(Employee employee, User employeeUser) {
        if (employeeUser != null && employeeUser.getDepartmentId() != null) {
            return departmentRepository.findById(employeeUser.getDepartmentId())
                    .orElseThrow(() -> new ResourceNotFoundException("Employee department not found with id: " + employeeUser.getDepartmentId()));
        }
        if (employee.getEmployeeDepartments() != null) {
            return employee.getEmployeeDepartments().stream()
                    .filter(ed -> ed.getEnddate() == null && ed.getDepartment() != null)
    ...
    private void ensureCycleTargetsDepartment(AppraisalCycle cycle, Integer departmentId) {
        boolean matched = cycle.getCycleDepartments() != null && cycle.getCycleDepartments().stream()
                .anyMatch(target -> target.getDepartment() != null && Objects.equals(target.getDepartment().getId(), departmentId));
        if (!matched) {
    ...
    private String resolveEmployeeName(Employee employee, User employeeUser) {
        if (employeeUser != null && employeeUser.getFullName() != null && !employeeUser.getFullName().isBlank()) {
    ...
    private void savePmRatings(EmployeeAppraisalForm form, List<AppraisalRatingInput> ratings) {
        List<AppraisalFormCriteria> templateCriteria = criteriaRepository.findActiveCriteriaByTemplateId(form.getCycle().getTemplate().getId());
        Map<Integer, AppraisalFormCriteria> criteriaById = templateCriteria.stream()
        ...
        for (AppraisalRatingInput input : ratings) {
            ...
            if (input.getRatingValue() < 1 || input.getRatingValue() > criteria.getMaxRating()) {
                throw new BadRequestException("Rating value must be between 1 and " + criteria.getMaxRating()
                        + " for criteria id: " + input.getCriteriaId());
            }
    ...
    private void recalculateScore(EmployeeAppraisalForm form) {
        List<EmployeeAppraisalCriteriaRating> ratings = ratingRepository.findByEmployeeAppraisalFormId(form.getId());
        int totalPoints = ratings.stream().mapToInt(EmployeeAppraisalCriteriaRating::getRatingValue).sum();
        int maxPossible = ratings.stream()
        ...
        double scorePercent = maxPossible > 0 ? (totalPoints * 100.0) / maxPossible : 0.0;
        ...
        form.setPerformanceLabel(scoreBandRepository.findBandForScore(roundedScore)
    ...
    private String resolveFallbackLabel(int roundedScore) {
        if (roundedScore >= 86) return "Outstanding";
        if (roundedScore >= 71) return "Exceeds Requirements";
        if (roundedScore >= 60) return "Meet Requirement";
        if (roundedScore >= 40) return "Need Improvement";
    ...
    private void upsertReview(
        ...
        AppraisalReview review = reviewRepository.findByEmployeeAppraisalFormIdAndReviewStage(form.getId(), stage)
    ...
    private EmployeeAppraisalFormResponse mapForm(EmployeeAppraisalForm form) {
        ...
        response.setReviews(reviewRepository.findByEmployeeAppraisalFormIdOrderByCreatedAtAsc(form.getId()).stream()
                .map(this::mapReview)
                .toList());
        return response;
    }
    ...
    private List<AppraisalSectionResponse> mapSectionsWithRatings(EmployeeAppraisalForm form) {
        ...
        Map<Integer, EmployeeAppraisalCriteriaRating> ratingsByCriteriaId = ratingRepository.findByEmployeeAppraisalFormId(form.getId()).stream()
                .collect(Collectors.toMap(rating -> rating.getCriteria().getId(), Function.identity()));

        return sectionRepository.findByTemplateIdWithCriteria(templateId).stream()
                ...
                .map(section -> {
                    ...
                    sectionResponse.setCriteria(section.getCriteria().stream()
                            ...
                            .map(criteria -> {
                                EmployeeAppraisalCriteriaRating rating = ratingsByCriteriaId.get(criteria.getId());
